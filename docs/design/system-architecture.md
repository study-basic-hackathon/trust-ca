...
本書は、2026年8月6日の定例MTGでの決定事項([docs/mtg/20260806.km](../mtg/20260806.km))を起点に、[ekyc-design.md](./ekyc-design.md)のeKYC設計、[docs/design/seller-onboarding-review-flow.md](./seller-onboarding-review-flow.md)の審査フロー、[docs/research/trustca-market-research.md](../research/trustca-market-research.md)の競合分析、および現行実装(`poc/ekyc/`)を統合し、プロダクト全体の**システムアーキテクチャ**を定義する。
...
現行の`poc/ekyc/`はこの図の「FE」と「BE」がNext.js 1プロセスに同居し、「DB」がSQLiteファイルになっている状態に相当する。

---

## 3. 5層信頼モデルとの対応

ekyc-design.md 1節の5層信頼モデルに、担当コンポーネントと実装状況を対応させる。

| 信頼層 | 確認内容 | 担当技術 | 実装状況(2026-08-21時点) | 該当コンポーネント |
|---|---|---|---|---|
| 人物の信頼 | 本人確認書類・顔照合・ライブネス・重複人物 | eKYC(Didit) | ✅ 実装済み(`backend/` + 運営者審査画面) | BE ⇔ Didit |
| アカウントの信頼 | 電話・メール・端末・IP | eKYC付帯機能+自社実装 | 🟨 一部のみ(Didit付帯のIP分析結果を保存。電話・メール確認、端末分析の自社実装は**未着手**) | BE + DB |
| 商品情報の信頼 | カード名・グレード・PSA Cert情報 | PSA Public API+OCR | ✅ 実装済み(PSA照会・永続化・カード紐付け、Vision OCR内容整合。※PSA API利用はアカウント承認待ち) | BE ⇔ PSA / Vision |
| 物理商品の信頼 | 所持確認・画像の同一性 | nonce再撮影+Vision API | 🟨 基盤のみ(画像種別`possession`・到着後アップロード用`arrival`はschema/APIに存在。**nonce付き所持証明フローと到着後再撮影比較のUI・業務フローは未実装** — 現時点で最大の機能ギャップ) | BE ⇔ Vision |
| 取引行動の信頼 | 価格・頻度・通報率 | 自社Risk Engine | ⬜ 未着手(静的なseller_limitsによる金額・件数上限のみ。価格乖離・頻度・通報率に基づく動的な審査分流は未実装) | BE + DB |
...
## 4. 現行PoC(`poc/ekyc/`)と目標アーキテクチャの差分
...
設計・実装済み。詳細はekyc-design.md 2節・3節と[seller-onboarding-review-flow.md](./seller-onboarding-review-flow.md)を参照。本書としては次の2点のみ補足する。
...
- データモデル(最小): `cards`(出品情報)、`orders`(取引)。`sellers`は既存の`poc/ekyc/`スキーマを流用。
- 出品は`seller_verifications.status = approved`(=`isSellingAllowed`)の販売者のみ許可 — ekyc-design.md 2.1原則4「eKYC合格 ≠ 無制限出品」に従い、金額上限・出品数制限は`sellers`または新設の`seller_limits`テーブルで管理する。
...
ekyc-design.md 3節の「物理商品の信頼」層で言及されているnonce再撮影の考え方をそのまま踏襲する。
...
ekyc-design.md 2.1節の設計原則は、機能が増えても崩さずに横展開する。
...
4. **段階的な信頼付与** — eKYC合格後も条件付き出品とする原則(ekyc-design.md 2.1原則4)を、新機能(カード販売・決済)の与信設計にも適用する。
...
ekyc-design.md 4.2節の優先順位に、本書で新たに設計した項目(カード販売・真贋チェック・ブロックチェーン・インフラ移行)を統合する。
