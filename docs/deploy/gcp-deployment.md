...
6. **画像bucketにはCORS設定が必須**。ブラウザは署名付きURLへ直接PUTするため、frontendのオリジンからの`PUT`(許可ヘッダ`Content-Type`)を許可するCORSをbucketへ設定する。未設定だとプリフライトで失敗し、アップロードがブラウザ側でブロックされる(Cloud Runのログには一切出ない)。設定コマンド: `gcloud storage buckets update gs://trust-ca-506116-card-images --cors-file=cors.json`(2026-08-22適用済み。オリジン: frontendの2つのrun.app URL + localhost:3000)。
